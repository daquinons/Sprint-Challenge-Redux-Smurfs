...
Store is the place where the common state tree is saved. It's a single source of truth because everyone gets its props directly from there and it's updated in a predictible way.

Reducers is where the result of the action arrives and the state is updated using immutable ways.

Actions are the functions executed and dispatched by the components.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state uses data that is important for the core functions and interest multiple components at the same time.
Component state would be something as small as keeping track of a checkbox. That small UI is not relevant for the whole application (it will be important the change in data that makes that checkbox).
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
Thunk is a middleware installed in redux so every action, before finalizing, passes through it. In the case of Thunk it gives the ability to perform and execute async calls (to databases, APIs, etc) because redux is sychronous by default.
## Project Set Up
